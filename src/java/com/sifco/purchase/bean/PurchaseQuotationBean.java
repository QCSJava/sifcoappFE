...
import com.sifcoapp.objects.purchase.to.PurchaseQuotationDetailTO;
import com.sifcoapp.objects.purchase.to.PurchaseQuotationInTO;
import com.sifcoapp.objects.purchase.to.PurchaseQuotationTO;
...
@ManagedBean(name = "purchaseQuotationBean")
@SessionScoped
public class PurchaseQuotationBean implements Serializable {
...
//<editor-fold defaultstate="collapsed" desc="Declaración de variables para formulario" >
    ...
    //__________________________________________________________________________
    //Nueva Factura
    private PurchaseQuotationTO newQuotation = new PurchaseQuotationTO();        //Nueva Factura
    private PurchaseQuotationTO selectPurchase = new PurchaseQuotationTO();        //Select de lista busqueda
    ...
    private PurchaseQuotationDetailTO selectDetail = new PurchaseQuotationDetailTO(); //detalle seleccionado para eliminar
    private ArrayList<PurchaseQuotationDetailTO> listaDetalles = new ArrayList<>(); //DataTable 
    ...
    private ArrayList<PurchaseQuotationTO> listaBusquedaTable = new ArrayList<>();
    ...
    public void accionAgregar(ActionEvent actionEvent) {
        try {
            if (validarNull()) {
                ...
                PurchaseQuotationDetailTO newDetalle = new PurchaseQuotationDetailTO();
                ArticlesTO thisArt = new ArticlesTO();
                CatalogTO thisCat = new CatalogTO();
                newDetalle.setObjtype(22 + "");
...
//<editor-fold defaultstate="collapsed" desc="funciones para calculos de impuestos">
    public Double calcularGravadas(ArrayList<PurchaseQuotationDetailTO> listaArt) {
        Double sumTotal = 0.0;
        for (PurchaseQuotationDetailTO art : listaArt) {
            sumTotal = sumTotal + art.getLinetotal();
        }
        return sumTotal;
    }

    public Double calcularExentas(ArrayList<PurchaseQuotationDetailTO> listaArt) {
        return 0.0;

    }

    public Double calcularIVA(ArrayList<PurchaseQuotationDetailTO> listaArt) {
        Double sumIVA = 0.0;
        //Double sumTotal = 0.0;
        for (PurchaseQuotationDetailTO det : listaArt) {
    ...
    public Double calcularRetencion(ArrayList<PurchaseQuotationDetailTO> listaArt) {
        return 0.0;
    }

    public Double calcularFovial(ArrayList<PurchaseQuotationDetailTO> listaArt) {
    ...
    public Double calcularCotrans(ArrayList<PurchaseQuotationDetailTO> listaArt) {
...
//<editor-fold defaultstate="collapsed" desc="Manejo de estados de la pantalla GUARDAR; ACTUALIZAR; BUSCAR; NUEVO" > 
    public void estateGuardar() {//Estado por defecto
        ...
        RequestContext.getCurrentInstance().update("frmQuotation");
    }
...
//<editor-fold defaultstate="collapsed" desc="GUARDAR EN BASE">
    public void doSave() {
        ...
        if (newQuotation == null) {
            newQuotation = new PurchaseQuotationTO();
        }
        newQuotation.setObjtype(22 + "");
        newQuotation.setDocstatus(estadoDoc);
        newQuotation.setUsersign((int) session.getAttribute("usersign"));
        newQuotation.setCardname(socioNeg);
        newQuotation.setCardcode(codSocio);
        ...
        newQuotation.setDocduedate(fechaEntre);
        newQuotation.setTaxdate(fechaDoc);

        newQuotation.setSeries(tipoDoc);
        newQuotation.setPeymethod("" + formaPago);
        newQuotation.setTowhscode(alm);
        newQuotation.setCtlaccount(ctlaccount);

        if (coment.equals("")) {
            newQuotation.setComments(vacio);
        } else {
            newQuotation.setComments(coment);
        }

        Double gTotalPadre = 0.0;
        Iterator<PurchaseQuotationDetailTO> iterator2 = listaPadre.iterator();
        ...
        newQuotation.setDoctotal(gTotalPadre);
        newQuotation.setPurchaseQuotationDetails(listaPadre);

        try {
            ResultOutTO _res;
            _res = PurchaseEJBClient.inv_PurchaseQuotation_mtto(newQuotation, Common.MTTOINSERT); //1 insert
...
//<editor-fold defaultstate="collapsed" desc="UPDATE EN BASE">
    public void doUpdate() {
        ...
        String vacio = null;
        newQuotation.setObjtype(22 + "");
        newQuotation.setDocentry(docEntry);
        newQuotation.setDocnum(docNum);
        newQuotation.setDocstatus(estadoDoc);
        ...
        try {
            ResultOutTO _res;
            _res = PurchaseEJBClient.inv_PurchaseQuotation_mtto(newQuotation, Common.MTTOUPDATE); //1 insert
...
//<editor-fold defaultstate="collapsed" desc="BUSCAR EN BASE">
    public void doSearch() {
        PurchaseQuotationInTO searchPurchase = new PurchaseQuotationInTO();
        ...
        searchPurchase.setDocduedate(fechaConta);
        ...
        try {
            listaBusqueda = PurchaseEJBClient.getPurchaseQuotation(searchPurchase);
        } catch (Exception e) {
        ...
        if (!listaBusqueda.isEmpty()) {
            if (listaBusqueda.size() == 1) {
                ...
                newQuotation = (PurchaseQuotationTO) listaBusqueda.get(0);
                try {

                    PurchaseQuotationTO var = PurchaseEJBClient.getPurchaseQuotationByKey(newQuotation.getDocentry());
                    llenarPantalla(var);
                    estateActualizar();
                } catch (Exception ex) {
                    Logger.getLogger(PurchaseQuotationBean.class.getName()).log(Level.SEVERE, null, ex);
                ...
                for (Object purchase : listaBusqueda) {
                    try {
                        PurchaseQuotationTO var = (PurchaseQuotationTO) purchase;
                ...
                showHideDialog("dlgListQuotation", 1);
...
//<editor-fold defaultstate="collapsed" desc="Funciones Varias">
    ...
    public void selectDialogBill() {
        ...
        showHideDialog("dlgListQuotation", 2);
        PurchaseQuotationTO var = (PurchaseQuotationTO) selectPurchase;

        try {
            newQuotation = PurchaseEJBClient.getPurchaseQuotationByKey(var.getDocentry());
            llenarPantalla(newQuotation);
            estateActualizar();
        } catch (Exception ex) {
            Logger.getLogger(PurchaseQuotationBean.class.getName()).log(Level.SEVERE, null, ex);
            faceMessage("Error en busqueda por PK");
        ...
        selectPurchase = new PurchaseQuotationTO();
        RequestContext.getCurrentInstance().update("frmQuotation");
    ...
    //llenar pantalla
    public void llenarPantalla(PurchaseQuotationTO var) {
        ...
        setFechaEntre(var.getDocduedate());
        ...
        for (Object detalle : var.getPurchaseQuotationDetails()) {
            PurchaseQuotationDetailTO det = (PurchaseQuotationDetailTO) detalle;
    ...
    public boolean validarNewBill() {
        ...
        if (fechaConta == null && fechaDoc == null && fechaEntre == null || fechaConta == null || fechaDoc == null || fechaEntre == null) {
            faceMessage("Seleccione todas las fechas");
            return false;
        }
...
//<editor-fold defaultstate="collapsed" desc="G & S">
    ...
    public static void setCatalogEJB(CatalogEJBClient CatalogEJB) {
        PurchaseQuotationBean.CatalogEJB = CatalogEJB;
    }
    ...
    public static void setAdminEJBService(AdminEJBClient AdminEJBService) {
        PurchaseQuotationBean.AdminEJBService = AdminEJBService;
    }
    ...
    public static void setPurchaseEJBClient(PurchaseEJBClient PurchaseEJBClient) {
        PurchaseQuotationBean.PurchaseEJBClient = PurchaseEJBClient;
    }
    ...
    public static void setAccountingEJBClient(AccountingEJBClient AccountingEJBClient) {
        PurchaseQuotationBean.AccountingEJBClient = AccountingEJBClient;
    }
    ...
    public PurchaseQuotationTO getNewQuotation() {
        return newQuotation;
    }

    public void setNewQuotation(PurchaseQuotationTO newQuotation) {
    ...
    public PurchaseQuotationTO getSelectPurchase() {
        return selectPurchase;
    }

    public void setSelectPurchase(PurchaseQuotationTO selectPurchase) {
    ...
    public PurchaseQuotationDetailTO getSelectDetail() {
        return selectDetail;
    }

    public void setSelectDetail(PurchaseQuotationDetailTO selectDetail) {
        this.selectDetail = selectDetail;
    }

    public ArrayList<PurchaseQuotationDetailTO> getListaDetalles() {
        return listaDetalles;
    }

    public void setListaDetalles(ArrayList<PurchaseQuotationDetailTO> listaDetalles) {
    ...
    public ArrayList<PurchaseQuotationTO> getListaBusquedaTable() {
        return listaBusquedaTable;
    }

    public void setListaBusquedaTable(ArrayList<PurchaseQuotationTO> listaBusquedaTable) {
